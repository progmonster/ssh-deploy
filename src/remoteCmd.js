...
const remoteCmd = async (content, privateKeyPath, isRequired, label) => new Promise((resolve, reject) => {
  ...
  try {
    ...
    exec(
      `DEBIAN_FRONTEND=noninteractive ssh -p ${(remotePort || 22)} -i ${privateKeyPath} -o StrictHostKeyChecking=no -o ConnectTimeout=15 -o ConnectionAttempts=3 ${sshServer} 'RSYNC_STDOUT="${rsyncStdout}" bash -s' < ${filename}`,
      (err, data = '', stderr = '') => {
        if (err) {
